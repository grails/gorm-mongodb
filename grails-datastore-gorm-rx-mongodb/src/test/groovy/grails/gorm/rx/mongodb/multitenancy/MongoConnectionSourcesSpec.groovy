...
import org.grails.datastore.rx.mongodb.connections.MongoConnectionSources
...
class MongoConnectionSourcesSpec extends Specification {
    ...
    void setupSpec() {
        ...
        for(name in client.listDatabaseNames().toList().toBlocking().first()) {
            client.getDatabase(name).drop().toBlocking().first()
        }
        def database = client.getDatabase("defaultDb")
        database.drop().toBlocking().first()
    ...
    void "Test persist and retrieve entities with multi tenancy"() {
        ...
        datastoreClient.connectionSources.addConnectionSource("test3",[url:"mongodb://localhost/test3Db"])
